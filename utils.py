...
class GAT(nn.Module):
    def __init__(self,
        ...
        if self.n_layers == 1:
            self.gats.append(MAGICGATConv(
                n_dim, 1, out_dim, n_heads_out, feat_drop, attn_drop, negative_slope,
                last_residual, norm=last_norm, concat_out=self.concat_out
            ))
        else:
            self.gats.append(MAGICGATConv(
                n_dim, 1, hidden_dim, n_heads, feat_drop, attn_drop, negative_slope,
                residual, create_activation(activation),
                norm=norm, concat_out=self.concat_out
            ))
            for _ in range(1, self.n_layers - 1):
                self.gats.append(MAGICGATConv(
                    hidden_dim * self.n_heads, 1, hidden_dim, n_heads,
                    feat_drop, attn_drop, negative_slope,
                    residual, create_activation(activation),
                    norm=norm, concat_out=self.concat_out
                ))
            self.gats.append(MAGICGATConv(
                hidden_dim * self.n_heads, 1, out_dim, n_heads_out,
                feat_drop, attn_drop, negative_slope,
...
class MAGICGATConv(nn.Module):
    def __init__(self,
                 ...
                 norm=None,
                 concat_out=True):
        super(MAGICGATConv, self).__init__()
        self.n_heads = n_heads
